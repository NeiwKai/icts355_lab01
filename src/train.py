...
def ensure_raw_data(cfg) -> None:
    """Download raw dataset via CloudAdapter if missing locally inside container."""
    local_path = Path(cfg.raw_path)
    if local_path.exists():
        print(f"Using existing local raw data at {local_path}")
        return

    print(f"Dataset missing at {local_path}. Fetching via CloudAdapter...")
    adapter = get_adapter(cfg)
    adapter.download("data/raw/sensors.csv", str(local_path))
    print(f"Successfully downloaded raw dataset to {local_path}")


def upload_metrics_artifact(cfg, local_path: str, destination_key: str) -> None:
    """Upload a local file artifact via CloudAdapter."""
    adapter = get_adapter(cfg)
    uploaded_uri = adapter.upload(local_path, destination_key)
    print(f"Uploaded {local_path} to {uploaded_uri}")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-gcs-path", type=str, default=None,
                   help="Remote destination key for final metrics.json")
    p.add_argument("--checkpoint-dir", type=str, default=None,
                   help="Remote path for Spot VM resumable checkpoints")
    return p.parse_args()
...
def main() -> None:
    ...
    # 1. Download raw data if missing using abstract cloud adapter
    ensure_raw_data(cfg)
